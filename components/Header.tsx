...
export default function Header() {
  ...
  return (
    <header
      className={`fixed w-screen bg-header-color h-20 px-32 flex items-center justify-between z-10 font-normal text-xl ${
    ...
    >
      ...
      <nav className="flex items-center flex-row divide-x-2 divide-solid divide-white text-white">
        {NAVIGATION.map((item) => (
          <div key={item.id} className={`px-6 ${pathname === item.path && 'text-black'}`}>
